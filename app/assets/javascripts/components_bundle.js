...
NewsCard = React.createClass({
  ...
  render: function() {
    return DOM.div({
      ...
      style: {
        minHeight: this.props.passedInDomProps.req_div_height
      }
    }, DOM.a({
...
NewsCardsContainer = React.createClass({
  ...
  createCards: function() {
    ...
    var card, element, i, j, len, ref, results;
...
NewsIndexPage = React.createClass({
  ...
  getInitialState: function() {
    return {
      articles: {
        ...
        articles_dom_props: this.initialArticlesDomProps()
      },
  ...
  initialArticlesDomProps: function() {
    var array, article, i, len, ref;
    array = [];
    ref = this.props.articles;
    for (i = 0, len = ref.length; i < len; i++) {
      article = ref[i];
  ...
  setDomValuesForArticle: function(current_art_dom_props, refs, current_article_id, current_card_number) {
  ...
  equalizeRows: function(articles, previous_art_dom_props) {
    var articles_in_same_row, heights_of_cards_in_same_row, required_height;
    articles_in_same_row = _.filter(articles.articles_dom_props, function(value) {
      if (value.pos_top === previous_art_dom_props.pos_top) {
        return value;
      }
    });
    heights_of_cards_in_same_row = _.pluck(articles_in_same_row, 'div_height');
    required_height = _.max(heights_of_cards_in_same_row);
    _.map(articles_in_same_row, function(value) {
      return value.req_div_height = required_height;
    });
    return articles;
  },
  equalizeRowsWrapper: function(articles, current_card_number, current_art_dom_props) {
    var previous_art_dom_props;
    if (current_card_number > 0) {
      previous_art_dom_props = _.find(articles.articles_dom_props, {
        card_number: current_card_number - 1
      });
      if (current_art_dom_props.pos_top > previous_art_dom_props.pos_top) {
        articles = this.equalizeRows(articles, previous_art_dom_props);
      }
    }
    return articles;
  },
  getDomPropsForArticle: function(refs, current_article_id, current_card_number) {
    var articles, current_art_dom_props;
    articles = this.state.articles;
    current_art_dom_props = _.find(articles.articles_dom_props, {
      article_id: current_article_id
    });
    articles.articles_states[current_article_id].needs_resizing = false;
    current_art_dom_props = this.setDomValuesForArticle(current_art_dom_props, refs, current_article_id, current_card_number);
    articles = this.equalizeRowsWrapper(articles, current_card_number, current_art_dom_props);
    return this.setState({
      articles: articles
    });
  },
  refreshArticles: function(articles) {
    _.forIn(articles.articles_states, function(value) {
      return value.needs_resizing = true;
    });
    _.map(articles.articles_dom_props, function(value) {
      value.pos_top = 0;
      value.div_height = 0;
      return value.req_div_height = 0;
    });
    return articles;
  },
